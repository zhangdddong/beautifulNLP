...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_optimizer if not any([nd in n for nd in no_decay])], 'wight_decay': 0.01},
    ...
    total_batch = 0     # 记录有多少个batch
    dev_best_loss = float('inf')   # 记录dev上的最好的loss，取float的最小
    last_imporve = 0    # 记录上一次校验集loss下降的batch数
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            if total_batch % 100 == 0:
                ...
                print(msg.format(total_batch, loss.item(), train_acc, dev_loss, dev_acc, time_idf, imporve))
                model.train()   # 评估完成后 需要重新设置
            total_batch += 1

            if total_batch - last_imporve > config.require_improvement:
                # 在验证集合上loss超过1000batch没有下降，结束训练。
                print('在校验集合上已经长时间没有提升了，模型自动停止训练。')
                flag = True
                break
        if flag:
            break
...
def evaluate(config, model, dev_iter, test=False):
    ...
    if test:
        report = metrics.classification_report(labels_all, target_names=config.class_list, digits=4)
